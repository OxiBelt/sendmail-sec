...
impl AppConfig {
    pub fn load(path: &Path) -> anyhow::Result<Self> {
        ...
        let mut config: AppConfig = match path.extension().and_then(|ext: &std::ffi::OsStr| ext.to_str()) {
        ...
        match &self.remote_smtp.auth {
            ...
            | RemoteAuthConfig::Xoauth2 {
                ...
                if username.is_empty() || access_token.is_empty() {
                    bail!(
                        "remote_smtp.auth OAuth mechanisms require non-empty username and access_token"
                    );
                }
            }
        }
