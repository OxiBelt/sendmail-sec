...
impl SmtpListener {
    ...
    loop {
      ...
      match command.as_str() {
        ...
        "HELO" => {
          ...
          state.reset_transaction();
          self
            .write_response(&mut reader, 250, &self.listen.banner)
            .await?;
        }
        "AUTH" => {
          if !state.helo {
            self
              .write_response(&mut reader, 503, "send EHLO/HELO first")
              .await?;
            continue;
          }
          if state.authenticated {
            self
              .write_response(&mut reader, 503, "already authenticated")
              .await?;
            continue;
          }
        ...
        "MAIL" => {
          ...
          if !state.authenticated {
            self
              .write_response(&mut reader, 530, "authentication required")
              .await?;
            continue;
          }
          ...
          let sender: String = match parse_path_argument(&argument, "FROM:") {
            ...
            Err(error) => {
              self
                .write_response(&mut reader, 501, &error.to_string())
                .await?;
              continue;
            }
          };
        ...
        "RCPT" => {
          if state.mail_from.is_none() {
            self
              .write_response(&mut reader, 503, "need MAIL before RCPT")
              .await?;
            continue;
          }
          ...
          self
            .write_response(&mut reader, 250, "recipient ok")
            .await?;
        }
        "DATA" => {
          if state.mail_from.is_none() || state.rcpt_to.is_empty() {
            self
              .write_response(&mut reader, 503, "need MAIL and RCPT before DATA")
              .await?;
            continue;
          }

          self
            .write_response(&mut reader, 354, "end data with <CRLF>.<CRLF>")
            .await?;
          let raw_message: Vec<u8> =
            read_message_data(&mut reader, self.listen.message_size_limit_bytes).await?;
          ...
          match self.openpgp.encrypt_message(&envelope, &raw_message).await {
            Ok(relay_message) => match self.remote.send_message(&relay_message).await {
              Ok(()) => {
                state.reset_transaction();
                self
                  .write_response(&mut reader, 250, "message relayed")
                  .await?;
              }
              Err(error) => {
                ...
                state.reset_transaction();
                self
                  .write_response(&mut reader, 554, "failed to relay message")
                  .await?;
              }
            },
            Err(error) => {
              ...
              state.reset_transaction();
              self
                .write_response(&mut reader, 554, "failed to encrypt message")
                .await?;
            }
          }
        }
        ...
        _ => {
          self
            .write_response(&mut reader, 502, "command not implemented")
            .await?;
        }
      }
    }
  ...
  async fn handle_auth(
    ...
    let Some(mechanism) = parts.next() else {
      self
        .write_response(reader, 501, "missing AUTH mechanism")
        .await?;
      return Ok(());
    };
    ...
    if !mechanism.eq_ignore_ascii_case("PLAIN") {
      self
        .write_response(reader, 504, "only AUTH PLAIN is supported")
        .await?;
      return Ok(());
    }
    ...
    match decode_auth_plain(&response) {
      Ok((username, password)) => {
        if username == self.listen.auth.username && password == self.listen.auth.password {
          ...
          self
            .write_response(reader, 235, "authentication successful")
            .await?;
        } else {
          self
            .write_response(reader, 535, "authentication failed")
            .await?;
        }
      }
      Err(error) => {
...
fn decode_auth_plain(value: &str) -> anyhow::Result<(String, String)> {
  ...
  let username: String =
    String::from_utf8(parts[1].to_vec()).context("AUTH username is not UTF-8")?;
  let password: String =
    String::from_utf8(parts[2].to_vec()).context("AUTH password is not UTF-8")?;
  Ok((username, password))
}
