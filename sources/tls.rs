    ...
    pub fn from_config(config: &TlsConfig) -> anyhow::Result<Self> {
        ...
        for path in &config.extra_root_certificates {
            let pem = fs::read(path).with_context(|| {
                format!("failed to read extra root certificate {}", path.display())
            })?;

            for cert in rustls_pemfile::certs(&mut &pem[..]) {
                let cert = cert.with_context(|| {
                    format!("failed to parse PEM certificate from {}", path.display())
                })?;
                root_store
                    .add(cert.clone())
                    .with_context(|| format!("invalid root certificate in {}", path.display()))?;
                http_roots.push(Certificate::from_der(cert.as_ref()).with_context(|| {
